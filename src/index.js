...
function initWindow(wnd) {
	...
	if (!windows.has(wnd.window)) {
		...
		let elements = wnd.document.querySelectorAll(
			"iframe, [sortable], [cloneables]"
		);
...
function initElements(elements) {
	for (let el of elements) initElement({ target: el });
}
...
function initElement({
	...
	if (!draggable) {
		...
		if (isDraggable != "false") draggable = true;
	...
	if (!handle) handle = target.getAttribute("drag-handle");

	if (!group) group = target.getAttribute("dnd-group-name");

	if (!exclude) exclude = target.getAttribute("dnd-exclude");
	...
	if (target.tagName == "IFRAME") {
		...
		if (init || sortable || cloneables) {
			...
			if (sortable)
				target.setAttribute(
					"sortable",
					target.getAttribute("sortable") || ""
				);
			if (cloneables)
				target.setAttribute(
					"cloneables",
					target.getAttribute("cloneables") || ""
				);
			if (nested) {
	...
	if (dragType) target.dnd["dragType"] = dragType;
...
function init(params) {
	...
	if (params.onDrag || params.onDrop) initFunctions.push(params);
}
...
observer.init({
	name: "dndAddedNodes",
	types: ["addedNodes"],
	selector: "[cloneables], [sortable]",
	callback: (mutation) => {
		if (mutation.target.tagName == "IFRAME")
			setTimeout(() => {
				initElement({ target: mutation.target });
			}, 2000);
		else initElement({ target: mutation.target });
	}
});
...
// 	name: "dndAttributes",
// 	types: ['attributes'],
// 	attributeFilter: ['cloneables', 'sortable'],
// 	callback: mutation => {
// 		if (mutation.target.tagName == 'IFRAME')
// 			setTimeout(() => {
// 				initElement({target: mutation.target});
